document.getElementById("varsForm").addEventListener("submit", async (e) => {
  ...
  try {
    const response = await fetch("/generate_elasticsearch_vars", {
      method: "POST",
      body: formData
    });
    ...
    a.href = url;
    a.download = "elasticsearch_vars.zip";
    document.body.appendChild(a);
    a.click();
    a.remove();

    document.getElementById("response").innerHTML =
      "<p style='color:green'>Arquivo elasticsearch_vars.zip gerado com sucesso!</p>";
  } catch (error) {
    document.getElementById("response").innerHTML =
